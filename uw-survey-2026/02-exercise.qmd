...
```{r}
max(fit_spatial$gradients)

fit_spatial <- run_extra_optimization(fit_spatial, nlminb_loops = 1)
...
1.  What are the other options for 'spatial'?
2.  What are the other options for 'spatiotemporal'?
...
3.  What is new in the output of `fit` compared to `fit_spatial`?
...
What if we compare this isotropic model to the anisotropic model index?

```{r}
p1 <- predict(fit_aniso, newdata = grid, return_tmb_object = TRUE)
index_a <- get_index(p1, area = grid$cell_area, bias_correct = TRUE)
index_a$Method <- "Bias correction aniso"

bind_rows(index_a, index_c) %>%
  ggplot(aes(year, est, fill = Method)) +
  geom_line(aes(colour = Method)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.4) +
  xlab("Year") +
...
1.  AIC indicated that the model with anisotropy was the better model, but what difference do you notice between the two indices?
